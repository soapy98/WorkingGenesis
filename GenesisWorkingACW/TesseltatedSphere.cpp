...
void TesseltatedSphere::CreateDeviceDependentResources()
{
	...
	auto createHS2Task = loadHS2Task.then([this](const std::vector<byte>& fileData)
		{
			DX::ThrowIfFailed(m_deviceResources->GetD3DDevice()->CreateHullShader(&fileData[0], fileData.size(), nullptr, &m_tesshullShader));
		});
	...
	auto createPlaneTask = (createPSTask && createDSTask && createHSTask && createVSTask && createHS2Task).then([this]() {
...
void TesseltatedSphere::SetViewProjectionMatrixConstantBuffer(DirectX::XMMATRIX& view, DirectX::XMMATRIX& projection)
{
...
void TesseltatedSphere::SetCameraPositionConstantBuffer(DirectX::XMFLOAT3& cameraPosition)
{
...
void TesseltatedSphere::Update(DX::StepTimer const& timer)
{
...
void TesseltatedSphere::ReleaseDeviceDependentResources()
{
