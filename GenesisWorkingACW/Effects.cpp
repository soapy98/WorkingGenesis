...
Effects::Effects(const std::shared_ptr<DX::DeviceResources>& deviceResources, std::wstring name) :m_deviceResources(deviceResources), m_position(0.f, 0.f, 10.5f), m_rotation(0.0f, 0.0f, 0.0f), m_scale(20.f, 20.f, 20.f), _name(name)
{
...
void Effects::Render()
{
	...
	//Buffer is not created on first run so will throw exception, run again and it works
	context->UpdateSubresource1(
		m_camBuffer.Get(),
		0,
		NULL,
...
void Effects::SetCameraPositionConstantBuffer(DirectX::XMFLOAT3& cameraPosition)
{
	m_camBufferData.position = cameraPosition;
	m_camBufferData.padding = 1;
}

void Effects::CreateDeviceDependentResources()
{

	auto loadVSTask = DX::ReadDataAsync(L"ImplicitVS.cso");
	auto loadPSTask = DX::ReadDataAsync(_name);
...
void Effects::SetViewProjectionMatrixConstantBuffer(DirectX::XMMATRIX& view, DirectX::XMMATRIX& projection)
{
...
void Effects::ReleaseDeviceDependentResources()
{
...
void Effects::Update(DX::StepTimer const& timer)
{
