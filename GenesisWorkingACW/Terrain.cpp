...
Terrain::Terrain(const std::shared_ptr<DX::DeviceResources>& deviceResources) :m_deviceResources(deviceResources), m_position(0.f, -6.f, 5.5f), m_rotation(0.0f, 0.0f, 0.0f), m_scale(1.f, 1.f, 1.f)
{
...
void Terrain::CreateDeviceDependentResources()
{
	...
	auto loadPSTask = DX::ReadDataAsync(L"TPS.cso");
...
void Terrain::SetViewProjectionMatrixConstantBuffer(DirectX::XMMATRIX& view, DirectX::XMMATRIX& projection)
{
...
void Terrain::SetCameraPositionConstantBuffer(DirectX::XMFLOAT3& cameraPosition)
{
	m_camBufferData.position = cameraPosition;
	m_camBufferData.padding = 1;
}

void Terrain::Update(DX::StepTimer const& timer)
{
