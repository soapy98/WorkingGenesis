...
BlackHole::BlackHole(const std::shared_ptr<DX::DeviceResources>& deviceResources) :m_deviceResources(deviceResources), m_position(0.f, 0.f, 10.5f), m_rotation(0.0f, 0.0f, 0.0f), m_scale(20.f, 20.f, 20.f)
{
...
void BlackHole::SetCameraPositionConstantBuffer(DirectX::XMFLOAT3& cameraPosition)
{
	m_camBufferData.position = cameraPosition;
	m_camBufferData.padding = 1;
}

void BlackHole::CreateDeviceDependentResources()
{

	auto loadVSTask = DX::ReadDataAsync(L"ImplicitVS.cso");
	auto loadPSTask = DX::ReadDataAsync(L"fRAC.cso");
...
void BlackHole::SetViewProjectionMatrixConstantBuffer(DirectX::XMMATRIX& view, DirectX::XMMATRIX& projection)
{
...
void BlackHole::ReleaseDeviceDependentResources()
{
...
void BlackHole::Update(DX::StepTimer const& timer)
{
